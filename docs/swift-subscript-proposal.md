...
### 1.3 Why not `yield`? (the DESIGN_SPEC tension, resolved)

[Decided] The DESIGN_SPEC §4.4.1 describes subscripts that *yield* a projection:
suspend, lend a temporary borrow (`let` or `inout`), and resume when the caller is
done — giving O(1) in-place element mutation with no aliasing and no exposed pointer.
Our proposal uses `get`/`set` and a read-compute-write desugar instead. Is this a
regression?

**Safety:** Both approaches preserve the same guarantee. `get`/`set` never creates a
reference to the collection interior. The read path copies out (`let`), the caller
mutates its local copy, and the write path writes back through `set`. At no point does
a pointer/reference to an interior element escape to user code. No aliasing, no
exposed pointer — identical to yield.

**Performance (where they differ):** Yield gives true O(1) *in-place* mutation.
`get`/`set` desugars `a[i] += 1` into three steps:

```
// a[i] += 1   (v0 get/set desugar)
let tmp = a[i]   // get: read-copy-out
tmp += 1          // compute on local
a[i] = tmp        // set: write-back
```

This is O(read + compute + write), not O(1). For `Int`, `Bool`, or small structs the
copy is negligible. For a struct field with many fields, or a large buffer element,
the copy is a real cost that yield avoids.

**Why get/set now, yield later:** Yield-based projection requires the full MVS
ownership machinery (Perceus compile-time refcounting, `inout` semantics on the
projected path) — that's the v1 foundation, and it doesn't exist yet. v0 has naive
memory; there's no exclusivity pass, no projection analysis, no Perceus. `get`/`set`
is mechanically simple (a desugar from parse → typeck → IR) and is trivially correct
without any ownership analysis:

```
// The compiler sees `a[i] += 1` and produces:
//   temp = __subscript_get_fn(a, i)
//   temp = add(temp, 1)
//   a   = __subscript_set_fn(a, i, temp)
```

**The path to yield (v1):** When Perceus lands in v1, `subscript` declarations gain
a third body kind:

```
subscript(i: Int) -> T {
    get(self)     { self.buf[i] }               // still works
    set(inout self, value: T) { self.buf[i] = value }  // still works
    yield(inout self) -> inout T { yield &self.buf[i] } // O(1) projection, v1+
}
```

`yield` is strictly additive. The `get`/`set` desugar path remains as the dynamic
dispatch fallback; yield is an optimization/generalization for in-place access. The
spec's statement — *"subscripts that yield rather than return"* — remains the end
state; v0's `get`/`set` is the practical interim that gets us there without blocking
on Perceus.

### 1.4 Related: why arrays/indexed types are Deferred

[Decided] The `get`/`set` subscript desugar correctly handles *user-defined types*
(`List<T>`, `Grid`, custom structs) because the compiler sees both a read subscript
and a write subscript and chains them. Raw `[T]` heap buffers have no subscript
declaration — no `List` impl with `subscript` — so there's nothing for the desugar to
call. This is the same gap the current v0 has (and why `list.ax` wraps `[T]`). We
continue to treat `heap_alloc(N)` / `buffer[i]` / `buffer[i] = x` as builtins until
we decide whether raw buffers get an stdlib impl or compiler-synthesized subscripts.
