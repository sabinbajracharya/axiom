...
use super::error::lower_error_set_def;
...
pub(super) fn lower_item(node: parser::SyntaxNode, ctx: &mut LowerCtx) -> Option<Item> {
    let kind = node.kind();
    if let Some(fn_def) = ast::FnDef::cast(node.clone()) {
        ...
        ctx.diag(HirDiagnostic::NotYetSupported {
            feature: format!("{kind:?}"),
            span: ctx.span_of(&parser::SyntaxNode::new_root(
                parser::GreenNodeBuilder::new().finish(),
            )),
        });
        None
    }
...
fn lower_subscript_def(s: &ast::SubscriptDef, ctx: &mut LowerCtx) -> SubscriptDef {
    ...
    // `self` is an explicit first parameter in subscripts.
    // `is_setter` is derived from the `self` convention: `inout self` → setter.
    let params = lower_params(s.param_list(), ctx);
    let is_setter = params
        .first()
        .map(|p| p.name == "self" && p.convention == CallingConvention::Inout)
        .unwrap_or(false);
    let body = s
        .body()
        .map(|b| lower_block(&b, ctx))
...
fn lower_use_tree(node: &ast::UseTree) -> UseTree {
    use parser::{SyntaxElement, SyntaxKind};
