...
use axiom_hir::{FnDef, HirId, Item};
...
impl<'a> Monomorphizer<'a> {
    ...
    fn visit_call(&mut self, call: &axiom_hir::CallExpr) {
        self.visit_call_inner(call);
    }

    fn visit_call_with_subst(&mut self, call: &axiom_hir::CallExpr, _subst: &Substitution) {
        self.visit_call_inner(call);
    }

    fn visit_call_inner(&mut self, call: &axiom_hir::CallExpr) {
        let callee_id = match &call.callee {
            axiom_hir::NameRef::Resolved(r) => r.def_id,
            axiom_hir::NameRef::Unresolved(_) => return,
        };
