...
impl Value {
    ...
    pub fn from_const(c: &ir::IrConst) -> Self {
        match c {
            ir::IrConst::Int(n) => Value::Int(*n),
            ir::IrConst::Float(f) => Value::Float(*f),
            ir::IrConst::Bool(b) => Value::Bool(*b),
            ir::IrConst::String(s) => Value::String(s.clone()),
            ir::IrConst::Unit => Value::Unit,
        }
    }
...
mod tests {
    ...
    fn test_value_from_const() {
        assert_eq!(Value::from_const(&ir::IrConst::Int(99)), Value::Int(99));
        assert_eq!(Value::from_const(&ir::IrConst::Unit), Value::Unit);
    }
