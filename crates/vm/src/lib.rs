...
//!
//! Takes an [`IrModule`](ir::Ir) and executes it by walking basic blocks,
...
impl Vm {
    ...
    pub fn run(&mut self) -> Result<Value, VmError> {
        ...
        let args: Vec<Value> = entry_fn
            .params
            .iter()
            .map(|_p| Value::from_const(&ir::IrConst::Unit))
            .collect();
...
mod tests {
    use super::*;
    use ir::{IrBlock, IrConst, IrFunction, IrInstr, IrParam, Reg, Terminator};

    /// Build a minimal Ir with one function containing the given instructions
    /// and a Return terminator.
    fn make_ir(instrs: Vec<IrInstr>, return_reg: Option<Reg>) -> ir::Ir {
        let next_reg = instrs
            .iter()
            .map(|i| match i {
                IrInstr::Const { dst, .. }
