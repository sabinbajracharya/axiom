...
fn type_name_from_ty(ty: &typecheck::Ty) -> Option<String> {
    match ty {
        typecheck::Ty::Struct(s) => Some(s.name.clone()),
        typecheck::Ty::Enum(e) => Some(e.name.clone()),
        typecheck::Ty::String => Some("String".to_string()),
        typecheck::Ty::Int => Some("Int".to_string()),
        typecheck::Ty::Float => Some("Float".to_string()),
        typecheck::Ty::Bool => Some("Bool".to_string()),
        ...
        typecheck::Ty::Instance(inst) => Some(inst.name.clone()),
        _ => None,
    }
}
...
pub(super) fn lower_index_read(
    base: Reg,
    base_ty: Option<&typecheck::Ty>,
    indices: &[Reg],
    ctx: &mut FnLowerCtx,
) -> Reg {
    let dst = ctx.fresh_reg();
    if matches!(base_ty, Some(typecheck::Ty::HeapBuffer(_))) {
