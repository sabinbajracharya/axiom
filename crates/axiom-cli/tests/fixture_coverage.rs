...
fn discover_stages() -> Vec<Stage> {
    ...
    if let Ok(entries) = fs::read_dir(&crates_dir) {
        for entry in entries.flatten() {
            ...
            // Extract the layer name: "axiom-lexer" → "lexer"
            let layer = match crate_name.strip_prefix("axiom-") {
                Some(l) => l.to_string(),
                None => continue,
            };
...
fn default_exclusions() -> BTreeSet<(String, String)> {
    ...
    let all_stages = ["lexer", "parser", "hir", "typeck", "ir", "vm", "corpus"];
