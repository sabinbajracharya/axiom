//! Diagnostic snapshots. Each `fixtures/errors/*.ax` is parsed and lowered;
//! its rendered HIR diagnostics are compared to the checked-in `.stderr`
//! golden. Regenerate with `UPDATE_SNAPSHOTS=1`.
//!
//! These fixtures are *expected* to produce diagnostics — but the HIR must
//! still lower (recovery never drops source).
...
use axiom_hir::{lower, HirDiagnostic};
...
fn diagnostic_snapshots() {
    ...
    for path in files {
        ...
        // An error fixture with zero diagnostics is a silent-accept bug.
        assert!(
            !got.is_empty(),
